...
async function findIdempotentDocument(workspaceSlugs = "", externalId = null) {
  if (!workspaceSlugs || !externalId) return null;
  ...
  for (const workspace of workspaces) {
    ...
    for (const document of documents) {
      ...
      if (metadata.externalId !== externalId) continue;
      return {
        ...metadata,
        id: document.docId,
        location: document.docpath,
      };
    }
  }
  return null;
  ...
  app.post(
    ...
    */
      try {
        ...
        if (!!addToWorkspaces) {
          const embedded = await Document.api.uploadToWorkspace(
            addToWorkspaces,
            documents?.[0].location
          );
          if (!embedded) {
            // The processed JSON is only a staging artifact. Remove it after an
            // embedding failure so API retries do not leave orphan files.
            const stagedPath = path.resolve(
              documentsPath,
              documents?.[0]?.location || ""
            );
            if (
              isWithin(documentsPath, stagedPath) &&
              fs.existsSync(stagedPath)
            )
              fs.rmSync(stagedPath);
            return response.status(500).json({
              success: false,
              error: "Document was extracted but could not be embedded.",
              documents: [],
            });
          }
        }
  ...
  app.post(
     ...
     */
      try {
        ...
        const existingDocument = await findIdempotentDocument(
          addToWorkspaces,
          metadata.externalId
        );
        if (existingDocument) {
      ...
      try {
        response.status(200).json({
          schema: {
            ...
            externalId: "string | nullable (idempotency key)",
            ...
            layoutOrder: "integer | nullable",
            bbox: "array[4] | nullable",
          },
        });
      } catch (e) {
        console.error(e.message, e);

...
class TextSplitter {
   ...
   * @param {number} [config.chunkSize = 512] - The token size of each chunk.
   * @param {number} [config.chunkOverlap = 50] - The token overlap between chunks.
  ...
  static buildHeaderMeta(metadata = {}) {
    ...
    const PLUCK_MAP = {
      ...
      page: {
        as: "page",
        pluck: (metadata) => metadata?.page ?? null,
      },
      section: {
        as: "section",
        pluck: (metadata) => metadata?.section || null,
      },
      contentType: {
        as: "contentType",
        pluck: (metadata) => metadata?.contentType || null,
      },
      documentType: {
        as: "documentType",
        pluck: (metadata) => metadata?.documentType || null,
      },
      extractionMethod: {
        as: "extractionMethod",
        pluck: (metadata) => metadata?.extractionMethod || null,
      },
    };
...
class RecursiveSplitter {
  constructor({ chunkSize, chunkOverlap, chunkHeader = null }) {
    ...
    this.engine = new RecursiveCharacterTextSplitter({
      chunkSize,
      chunkOverlap,
      lengthFunction: (text) => tokenManager.countFromString(text),
    });
  }

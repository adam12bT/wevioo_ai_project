...
swaggerAutogen(outputFile, endpointsFiles, doc)
  .then(({ data }) => {
    if (!data) {
      console.error(
        "[Swagger] Generation returned no data — skipping openapi.json update. This does not affect the running API server."
      );
      return;
    }
  ...
  .catch((error) => {
    console.error(
      "[Swagger] Doc generation failed — this does not affect the running API server, only the /api-docs UI and openapi.json file.",
      error.message
    );
  });

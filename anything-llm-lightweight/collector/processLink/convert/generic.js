...
// YoutubeTranscript extension was removed in this lightweight fork.
// This project only processes uploaded PDF/DOCX files, not URLs/YouTube links,
// so this stub exists purely to prevent a startup crash if that code path is ever reached.
async function loadYouTubeTranscript() {
  throw new Error(
    "YouTube link processing is not available in this build. Only file uploads (PDF/DOCX) are supported."
  );
}
